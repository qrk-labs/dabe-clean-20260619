...
from src.training.fp16_chunk_feasibility import run_fp16_chunk_feasibility

log = logging.getLogger(__name__)


@hydra.main(version_base=None, config_path="../configs", config_name="fp16_chunk_feasibility_smoke")
def main(cfg: DictConfig) -> None:
    config = OmegaConf.to_container(cfg, resolve=True)
    experiment_cfg = config.get("experiment", {})
    experiment_name = str(experiment_cfg.get("name", "fp16_chunk_feasibility_smoke"))
    ...
    fp16_result = run_fp16_chunk_feasibility(
        config=config,
    ...
    results = {
        ...
        "track": "fp16_chunk_feasibility",
        "config": config,
        "fp16_chunk": fp16_result,
    }
