...
The main experiments use TinyStories as a controlled text domain with exact GPT-2-token reconstruction metrics. This setting makes it possible to compare architectural variants under the same token basis, chunk length, training budget, and validation protocol. Domain generalization remains a separate evaluation target; this setting establishes a clean mechanism result that should next be tested on broader corpora, code, multilingual data, and full LM training. EXP-097 makes this boundary explicit: the EXP-087 TinyStories-trained checkpoint reaches only `0.41761` token accuracy on a deterministic Python-code corpus, so the current evidence should not be read as zero-shot code-tokenizer performance. EXP-099 and EXP-100 show that code-domain training can produce favorable completion and reconstruction results on the same deterministic code generator, but that generator is intentionally small and repetitive; broad code claims require real Python corpora and held-out repository-level evaluation.
