...
EXP-099 adds a completion-level view of the same code-domain control. On the deterministic validation stream, DABE reaches `0.98964` next-token accuracy versus `0.98012` for the GPT-2 BPE baseline. The examples are qualitatively different: DABE predicts whole learned code spans such as `add_user(users,`, `name):`, and `users.append({'name':`, while BPE predicts subword pieces such as ` add`, `_`, and `user`.
