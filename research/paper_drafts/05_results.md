...
The inference overhead headline is concrete: sparse repair costs about `2.45x` decode throughput and `2.38x` peak allocated memory relative to the fixed-rate baseline on the matched T4 diagnostic path. The zero-shot code diagnostic is intentionally framed as scope evidence: the TinyStories-trained repair mechanism recognizes code as difficult (`active K=16.0`) but does not reconstruct code well without code-domain training.
